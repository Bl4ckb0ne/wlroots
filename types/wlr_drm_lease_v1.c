...
static void drm_lease_request_v1_handle_submit(
	...
	wl_resource_set_implementation(lease_resource, &lease_impl, lease,
...
static void drm_lease_device_v1_handle_create_lease_request(
	...
	if (!device) {
		wlr_log(WLR_ERROR, "Failed to create lease request, "
...
static void lease_device_bind(struct wl_client *wl_client, void *data,
	...
	if (!device) {
		wlr_log(WLR_ERROR, "Failed to bind lease device, "
